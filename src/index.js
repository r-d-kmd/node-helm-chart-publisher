...
(async () => {
	await prepare({ gitUrl: gitChartRepository.url, destination: workDir.clonedGitChartRepositoryPath });
	await build({ chartPath: chart.definitionPath, chartVersion: chart.version, packageDestinationPath: workDir.packageDestinationPath });
	await publish({
